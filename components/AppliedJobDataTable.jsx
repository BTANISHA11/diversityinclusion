...
export default function AppliedJobDataTable() {
    ...
    const columns = [
        ...
        {
            name: 'Status',
            selector: row => <p className={`uppercase font-semibold ${row?.status === "approved" ? "text-green-500" : ""} ${row?.status === "Pending" ? "text-yellow-500" : ""} ${row?.status === "rejected" ? "text-red-600" : ""}`}>{row?.status}</p> ,
        },
        {
            name: 'Action',
